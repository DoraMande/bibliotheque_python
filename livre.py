#PAR BOLIBENGA WETOKYA JONATHAN

import json
#Trois fonctions pour valider les informations de livre : validerTitre, validerAuteur, validerGenre
def validerTitre(titre):
    if not titre.startswith(("#", "@", "$", "%", "&", "_ ")) or len(titre) > 5 and len(titre) < 15:
        return True
    else:
        return False

def validerAuteur(auteur):
    if len(auteur) > 6 and len(auteur) < 20:
        return True
    else:
        return False

def validerGenre(genre):
    if genre == "roman" or genre == "bd" or genre == "historique" or genre == "mangas":
        return True
    else:
        return False
...
def ajouterLivre():
    ...
    for livre in livres:
        if livre["Titre"] == titreLivre and livre["Auteur"] == auteurLivre and livre["Genre"] == genrelivre:
            livre_existe = True
            break
    #Si le livre est valide, elle l'ajoute à la liste des livres dans le fichier JSON
    if not livre_existe:
        livre = {
            "id": idLivre,
            "Titre" : titreLivre,
...
def aficher_livres(): 
    try:
        ...
        def modifierLivre():
            livre_id = input("Entrez l'ID du livre à modifier: ")
            with open("livres.json", "r") as f:
                livres = json.load(f)
            index = -1
            for i in range(len(livres)):
                if livres[i]["id"] == int(livre_id):
                    index = i
                    break
            if index != -1:
                livre = livres[index]
                print("1. Modifier le titre")
                print("2. Modifier l'auteur")
                print("3. Modifier le genre")
                choix = input("Entrez votre choix: ")
                if choix == "1":
                    nouveau_titre = input("Entrez le nouveau titre: ")
                    if validerTitre(nouveau_titre):
                        livre["Titre"] = nouveau_titre
                        print("Titre modifié avec succès.")
                    else:
                        print("Le nouveau titre ne remplit pas les critères de la bibliothèque.")
                elif choix == "2":
                    nouveau_auteur = input("Entrez le nouveau nom de l'auteur: ")
                    if validerAuteur(nouveau_auteur):
                        livre["Auteur"] = nouveau_auteur
                        print("Auteur modifié avec succès.")
                    else:
                        print("Le nouveau nom de l'auteur ne remplit pas les critères de la bibliothèque.")
                elif choix == "3":
                    nouveau_genre = input("Entrez le nouveau genre: ")
                    if validerGenre(nouveau_genre):
                        livre["Genre"] = nouveau_genre
                        print("Genre modifié avec succès.")
                    else:
                        print("Le nouveau genre ne remplit pas les critères de la bibliothèque.")
                else:
...
def archiverLivre():
    ...
    if index != -1:
        ...
        print("Livre non trouvé.")
#La fonction suprimer_ou_archiver permet de choisir entre supprimer ou archiver un livre
